...
Each one has _slightly_ different approaches to development, but they all share
...
Much of what _was_ Iodide is now available in [starboard.gg](https://starboard.gg/). You can utilize "Offline" notebooks to have them stored _only_ in your browser, which is a notable improvement from the original Iodide.
...
```javascript
await import("https://cdn.jsdelivr.net/npm/vega@5");
await import("https://cdn.jsdelivr.net/npm/vega-lite@5");
await import("https://cdn.jsdelivr.net/npm/vega-embed@6");
```
...
This means that when you access it, it may not _yet_ be available -- so instead
of actually calling on it, we supply a function to be called _at some point_
...
```javascript
var cs = vega.changeset().remove(function (t) {
...
document
  ...
  .addEventListener("click", function onButtonClick(event) {});
```
...
Similarly, we can _remove_ data points by supplying a function that is
...
_function_ to it, rather than supplying a value. For instance, _both_ of these
